...
namespace EpidimiologyReportServer.Services
{
    public class UserService
    {
        ...
        public async Task DeleteReport(int patientId, Report report)
        {
            ...
            if (patientIndex != -1)
            {
                for (int i = 0; i < allPatients[patientIndex].Reports.Count(); i++)
                {
                    if (allPatients[patientIndex].Reports[i].City.Equals(report.City) &&
                        allPatients[patientIndex].Reports[i].Location.Equals(report.Location) &&
                        DateTime.Compare(allPatients[patientIndex].Reports[i].StartDate, report.StartDate) == 0 &&
                         DateTime.Compare(allPatients[patientIndex].Reports[i].EndDate, report.EndDate) == 0)
                    {

                        allPatients[patientIndex].Reports.RemoveAt(i);
                        break;
                    }
